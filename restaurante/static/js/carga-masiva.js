...
function updateJSONPreview() {
    ...
    document.getElementById('json-count').textContent =
        `${parsedData.length} productos cargados · mostrando primeros ${Math.min(3, parsedData.length)}`;
}
...
function validateData() {
    ...
    const required = ['nom_produ','des_produ','stock_actual_produ','stock_minimo_produ','precio_uni_produ','estado_produ','id_provee_produ_fk','id_uni_medi_produ_fk','id_cate_produ_fk'];
    let errCount = 0;
    ...
    if (errCount === 0) {
        alert(`✅ Validación OK — ${parsedData.length} productos listos para enviar`);
    } else {
...
async function sendOneByOne(url, headers, delay, startTime) {
    for (let i = 0; i < parsedData.length; i++) {
        ...
        updateProgress(i, parsedData.length, `Enviando ${i+1} / ${parsedData.length}: ${row.nom_produ}`);
        try {
            const res = await fetch(url, {
                method: httpMethod,
                headers,
...
function exportLogCsv() {
    ...
    const header = '#,Producto,Status HTTP,Resultado,Mensaje';
